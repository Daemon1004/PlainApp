...
import com.example.plainapp.webrtc.audio.AudioDevice
...
class AudioSwitchHandler constructor(private val context: Context) : AudioHandler {
  ...
  override fun start() {
    Log.d(TAG, "[start] audioSwitch: $audioSwitch")
    if (audioSwitch == null) {
  ...
  companion object {
    ...
    private val defaultAudioDeviceChangeListener by lazy(LazyThreadSafetyMode.NONE) {
      object : AudioDeviceChangeListener {
        override fun invoke(
          ...
          Log.i(TAG, "[onAudioDeviceChange] selectedAudioDevice: $selectedAudioDevice")
        }
      }
    }
    ...
    private class DefaultOnAudioFocusChangeListener : AudioManager.OnAudioFocusChangeListener {
      override fun onAudioFocusChange(focusChange: Int) {
        ...
        Log.i(TAG, "[onAudioFocusChange] focusChange: $typeOfChange")
      }
    }
  }
}
