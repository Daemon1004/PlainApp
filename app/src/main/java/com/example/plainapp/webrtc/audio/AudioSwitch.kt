...
import com.example.plainapp.webrtc.audio.AudioDevice.BluetoothHeadset
import com.example.plainapp.webrtc.audio.AudioDevice.Earpiece
import com.example.plainapp.webrtc.audio.AudioDevice.Speakerphone
import com.example.plainapp.webrtc.audio.AudioDevice.WiredHeadset
...
class AudioSwitch internal constructor(
  ...
  fun start(listener: AudioDeviceChangeListener) {
    Log.d(this::class.java.name,"[start] state: $state")
   ...
   */
  fun stop() {
    Log.d(this::class.java.name,"[stop] state: $state")
    when (state) {
      State.ACTIVATED -> {
        deactivate()
        closeListeners()
   ...
   */
  fun activate() {
    Log.d(this::class.java.name,"[activate] state: $state")
    when (state) {
      State.STARTED -> {
  ...
  private fun deactivate() {
    Log.d(this::class.java.name,"[deactivate] state: $state")
    when (state) {
      State.ACTIVATED -> {
  ...
  private fun selectDevice(audioDevice: AudioDevice?) {
    Log.d(this::class.java.name,"[selectDevice] audioDevice: $audioDevice")
  ...
  private fun activate(audioDevice: AudioDevice) {
    Log.d(this::class.java.name,"[activate] audioDevice: $audioDevice")
    when (audioDevice) {
  ...
  private fun enumerateDevices(bluetoothHeadsetName: String? = null) {
    Log.d(this::class.java.name,"[enumerateDevices] bluetoothHeadsetName: $bluetoothHeadsetName")
    ...
    Log.v(this::class.java.name,"[enumerateDevices] selectedDevice: $selectedDevice")
  ...
  private fun addAvailableAudioDevices(bluetoothHeadsetName: String?) {
    ...
    preferredDeviceList.forEach { audioDevice ->
      Log.v(this::class.java.name,"[addAvailableAudioDevices] audioDevice: ${audioDevice.simpleName}")
      when (audioDevice) {
        ...
        Speakerphone::class.java -> {
          ...
          Log.v(this::class.java.name,"[addAvailableAudioDevices] #Speakerphone; hasSpeakerphone: $hasSpeakerphone")
          if (hasSpeakerphone) {
